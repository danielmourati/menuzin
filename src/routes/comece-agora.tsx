      ...
      <main>
        <section className="relative overflow-hidden text-primary-foreground" style={{ background: "var(--gradient-hero)" }}>
          <div className="container mx-auto grid items-center gap-6 sm:gap-8 lg:gap-10 px-4 py-8 sm:py-12 md:py-16 lg:py-10 xl:py-16 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/30 bg-primary-foreground/15 px-3 py-0.5 text-xs font-bold text-primary-foreground sm:py-1">
              ...
              <div className="mt-5 sm:mt-6 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                <Button size="lg" variant="secondary" onClick={openSignup} className="w-full bg-background text-primary shadow-[var(--shadow-pop)] hover:bg-background/90 sm:w-auto">
                  Testar o Pro por 14 dias <ArrowRight aria-hidden="true" />
                </Button>
                <a href="#recursos" className="px-3 py-2 text-sm font-semibold text-primary-foreground/85 hover:text-primary-foreground">Ver como funciona</a>
              </div>
              <ul className="mt-4 sm:mt-5 flex flex-wrap gap-x-5 gap-y-2 text-xs sm:text-sm text-primary-foreground/90">
                {["Sem pedir cartão", "0% de comissão", "Retorno grátis ao Presença"].map((item) => (
                  <li key={item} className="flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-primary-foreground" aria-hidden="true" />{item}</li>
                ))}
              </ul>
            </div>
            <ProductHeroVisual />
